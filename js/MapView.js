...
function ProcessMapData(gamedata) {
    ...
    var phase = game.GamePhases[7];
    board.GamePhase = phase;

    board.AllowEdits = true;

    // game.GamePhases.forEach(gp => {
    //     let jj = gp.ToJSON();
    //     console.log(gp.Phase, JSON.stringify(jj) == JSON.stringify(gp.ToJSON(gmGamePhase.FromJSON(jj))));
    // });

    // console.log(board.MapView.MapData.AllSeaNeighbors, board.MapView.MapData.AllLandNeighbors);
    // return;

    game.Adjudicator = new gmAdjudicator(board.MapView.MapData);
    let newgp = game.Adjudicator.AdjudicateOrders(phase);

    //    console.log(board.MapView.MapData);
}

function RunDATC() {
    let datcs = [new gmAdjudicationTestCase6A()
        , new gmAdjudicationTestCase6B1()
        , new gmAdjudicationTestCase6B2()
        , new gmAdjudicationTestCase6B3()
        , new gmAdjudicationTestCase6C1()
    ];

    for (const datc of datcs) {
        datc.Adjudicator = new gmAdjudicator(myBoard.MapView.MapData);
        datc.Adjudicate();
        if (!datc.VerifyResults()) {
            myBoard.GamePhase = datc.GamePhase;
            datc.GamePhase.ConsoleLogOrderReport();
            return;
        }
    }

    myBoard.GamePhase = datcs[datcs.length - 1].GamePhase;

    datcs = [new gmAdjudicationTestCaseCoastsRequired()];
    for (const datc of datcs) {
        datc.Adjudicator = new gmAdjudicator(myBoard.MapView.MapData);
        datc.Adjudicator.RequireCoastDesignationForUnambiguousMoveOrders = true;
        datc.Adjudicator.RequireCoastDesignationForSupportMoveOrders = true;
        datc.Adjudicate();
        if (!datc.VerifyResults()) {
            myBoard.GamePhase = datc.GamePhase;
            datc.GamePhase.ConsoleLogOrderReport();
...
//#region Number array utilities (SHOULD BE DEPRECATED FOR dbnPoint and dbnLineSegment)
/**
 * 
 * @param {number[]} pt0 
 * @param {number[]} pt1 
...
class dbnProvinceData {
    ...
    constructor(province, json) {
        ...
        if ("Neighbors" in json) Object.entries(json["Neighbors"]).forEach(x => this.Neighbors[x[0]] = x[1].map(gmUnitWithLocation.FromJSON));
...
class dbnMapData {
    ...
    GetCoastOptions(province) {
        switch (province) {
            case ProvinceEnum.Stp: return [ProvinceCoastEnum.nc, ProvinceCoastEnum.sc];
            case ProvinceEnum.Bul: return [ProvinceCoastEnum.ec, ProvinceCoastEnum.sc];
            case ProvinceEnum.Spa: return [ProvinceCoastEnum.nc, ProvinceCoastEnum.sc];
            default: return null;
        }
    }
    ...
    /**@type{Object.<string,gmUnitWithLocation[]>} */
    #AllNeighborsByKey;
    /**@description keys are gmUnitWithLocation.Key */
    get AllNeighborsByKey() {
        if (!this.#AllNeighborsByKey) {
            this.#AllNeighborsByKey = {};
            Object.values(this.ProvinceData).forEach(pd => {
                Object.entries(pd.Neighbors).forEach(x => this.#AllNeighborsByKey[x[0]] = x[1]);
            });
        }
        return this.#AllNeighborsByKey;
    }

    /**@type{Object.<string,string[]>} */
    #AllSeaNeighbors;
    /**@description keys are provinces, values are arrays of provinces */
    get AllSeaNeighbors() {
        if (!this.#AllSeaNeighbors) {
            this.#AllSeaNeighbors = {};
            Object.values(this.ProvinceData).forEach(pd => {
                /**@type{gmUnitWithLocation[]} */
                let neighbors = [];
                let uwlFrom = new gmUnitWithLocation(UnitTypeEnum.Fleet, new gmLocation(pd.Province));
                let coasts = this.GetCoastOptions(pd.Province);
                if (coasts) {
                    coasts.forEach(coast => {
                        let thisUWL = new gmUnitWithLocation(UnitTypeEnum.Fleet, new gmLocation(pd.Province, coast));
                        neighbors.push(...pd.Neighbors[thisUWL.Key]);
                    });
                } else {
                    neighbors = pd.Neighbors[uwlFrom.Key];
                }
                if (neighbors) {
                    /**@type{string[]} */
                    let provinces = [];
                    neighbors.filter(x => this.ProvinceData[x.Location.Province].ProvinceType == ProvinceTypeEnum.Water).forEach(x => { if (!provinces.includes(x.Location.Province)) provinces.push(x.Location.Province) });
                    this.#AllSeaNeighbors[uwlFrom.Location.Province] = provinces;
                }
            });
        }
        return this.#AllSeaNeighbors;
    }

    /**@type{Object.<string,string[]>} */
    #AllLandNeighbors;
    /**@description keys are provinces, values are arrays of provinces */
    get AllLandNeighbors() {
        if (!this.#AllLandNeighbors) {
            this.#AllLandNeighbors = {};
            Object.values(this.ProvinceData).forEach(pd => {
                /**@type{gmUnitWithLocation[]} */
                let neighbors = [];
                let uwlFrom = pd.GetDefaultUnitWithLocation();
                neighbors = pd.Neighbors[uwlFrom.Key];
                this.#AllLandNeighbors[pd.Province] = neighbors.filter(x => this.ProvinceData[x.Location.Province].ProvinceType == ProvinceTypeEnum.Land).map(x => x.Location.Province);
            });
        }
        return this.#AllLandNeighbors;
    }
...
class dbnFullBoard extends dbnDiv {
    ...
    #ActivateOrderWriter = false;
    get ActivateOrderWriter() { return this.#ActivateOrderWriter; }
    ...
    constructor(parent) {
        ...
        if (this.ActivateOrderWriter) this.#OrderWriter.BindToMapView(this.#MapView);
    }
    ...
    get AllowEdits() { return this.#MapView.AllowEdits; }
    set AllowEdits(value) { this.MapView.AllowEdits = value; this.#OrderWriter.Active = value; }
}

//#endregion

//#region OrderListView

class dbnMapOrderListView extends dbnDiv {
    constructor() {
        super();
    ...
    #UpdateDisplay() {
        ...
        if (!this.GamePhase) {
            this.addText("No orders");
            return;
        }

        Object.entries(this.GamePhase.Orders).forEach(([country, oars]) => {
            var thisdiv = this.addDiv();
            thisdiv.style.backgroundColor = myHub.ColorScheme.CountryBackColors[country].ToRGBString();
            thisdiv.style.display = "table-row";
            thisdiv.style.border = "1px solid black";
            //thisdiv.style.width = "100%";
            oars.sort((a, b) => a.Province.localeCompare(b.Province));

            var makecolumn = () => {
                var ret = thisdiv.addDiv();
                ret.style.backgroundColor = myHub.ColorScheme.CountryBackColors[country].ToRGBString();
                ret.style.display = "table-cell";
            ...
            var col = makecolumn();
            col.addText(country.substring(0, 1));
            col.style.fontSize = "15px";
            const colcount = 3;

            var collength = Math.ceil(oars.length / colcount);

            oars.forEach((oar, i) => {
                if (i % collength == 0) col = makecolumn();
                let tspan = col.addSpan();
                tspan.addText(oar.ToString());
                tspan.style.textDecorationThickness = "3px";
                tspan.style.textDecorationColor = "red";
                tspan.domelement.onmouseenter = (() => { tspan.style.textDecoration = "line-through"; tspan.style.cursor = "pointer"; });
                tspan.domelement.onmouseleave = (() => { tspan.style.textDecoration = "none"; tspan.style.cursor = "default"; });
                tspan.onclick = () => {
                    if (this.GamePhase) {
                        this.GamePhase.RemoveOrderIfExists(country, oar);
                        this.#UpdateDisplay();
                        let newgp = this.MapView.Game.Adjudicator.AdjudicateOrders(this.MapView.GamePhase);
                        this.MapView.GamePhase = this.GamePhase;
                    }
                };
            ...
            while (thisdiv.domelement.childNodes.length < colcount + 1) { makecolumn(); }
...
class dbnOrderWriter {
    ...
    /**@type{dbnSVGText} */ #OrderSVG;
    /**@type{dbnSVGText} */ #TextSVG;
    /**@type{dbnSVGButtonWithText} */ #ReplaceExistingButton;

    /**@type{Object.<string,dbnSVGButton>} */ #CommandButtons;
    /**@type{Object.<string,dbnSVGButton>} */ #CountryButtons;
    /**@type{Object.<string,dbnSVGButton>} */ #ToCoastButtons;

    /**@type{string} */ PreviousCountry = CountryEnum.Austria;
    /**@type{string} */ SelectedCountry;
    /**@type{string} */ SelectedCommand;
    /**@type{string[]} */ SelectedProvinces;
    /**@type{string} */ SelectedToCoast;

    /**@type{boolean} */ ReplaceExisting = true;
    ...
    BindToMapView(mapview) {
        ...
        mapview.OnGameSet.AddListener(this.#MapViewGameSet.bind(this));
        mapview.OnGamePhaseSet.AddListener(this.#MapViewGamePhaseSet.bind(this));
        mapview.OnProvinceSelect.AddListener(this.#MapViewProvinceSelected.bind(this));
        mapview.OnDraw.AddListener(this.#MapViewDraw.bind(this));
    }

    #GetMaxProvincesLengthForCommand() {
        let maxlength = 1;
        switch (this.SelectedCommand) {
            case "h": break;
            case "m": maxlength = 2; break;
            case "sm": maxlength = 3; break;
            case "sh": maxlength = 2; break;
            case "c": maxlength = 3; break;
            case "X": maxlength = 0; break;
    ...
    #MapViewDraw() {
        this.#OrderSVG = null;
        this.#TextSVG = null;
        this.#ReplaceExistingButton = null;
        this.#CommandButtons = null;
        this.#CountryButtons = null;
        this.#ToCoastButtons = null;
        this.#UpdateDisplay();
    }

    #MapViewProvinceSelected(province) {
        if (!this.SelectedProvinces) {
            this.SelectedProvinces = [];
            this.SelectedCommand = "m";
        }
        if (this.SelectedProvinces.length < this.#GetMaxProvincesLengthForCommand()) this.SelectedProvinces.push(province);
        this.#UpdateDisplay();
    }

    #ClearOrder() {
        this.PreviousCountry = this.SelectedCountry ?? this.PreviousCountry;
        this.SelectedCountry = null;
        this.SelectedCommand = null;
        this.SelectedProvinces = null;
        this.SelectedToCoast = null;
    }
    ...
    #FinishOrder(oar) {
        if (this.ReplaceExisting) {
            let existing = this.GamePhase.GetOrdersForProvince(this.SelectedProvinces[0]);
            if (existing)
                Object.entries(existing).forEach(
                    x => x[1].forEach(
                        oar => this.GamePhase.RemoveOrderIfExists(x[0], oar)
                    )
                );
        }

        oar.Result = new gmOrderResult(true, "");//KLUGE
        this.MapView.GamePhase.Orders[this.SelectedCountry].push(oar);
        this.#ClearOrder();
        this.MapView.GamePhase = this.MapView.GamePhase;
        // this.#UpdateDisplay();
    ...
    #KeyPressed(key) {
        ...
        switch (key.key.toLowerCase()) {
            case "a": this.SelectedCountry = CountryEnum.Austria; break;
            case "e": this.SelectedCountry = CountryEnum.England; break;
            case "f": this.SelectedCountry = CountryEnum.France; break;
            case "g": this.SelectedCountry = CountryEnum.Germany; break;
            case "i": this.SelectedCountry = CountryEnum.Italy; break;
            case "r": this.SelectedCountry = CountryEnum.Russia; break;
            case "t": this.SelectedCountry = CountryEnum.Turkey; break;

            case "escape":
            case "backspace": this.#ClearOrder(); break;

            case "m": this.SelectedCommand = "m"; break;
            case "h": this.SelectedCommand = "h"; break;
            case "c": this.SelectedCommand = "c"; break;
            case "s": this.SelectedCommand = this.SelectedCommand == "sm" ? "sh" : "sm"; break;
    ...
    #UpdateDisplay() {
        if (!this.MapView || !this.MapView.GamePhase) return;

        let text = "", color = "", backcolor = "";

        if (this.SelectedCommand && this.SelectedProvinces && this.SelectedProvinces.length > 0) {

            if (this.SelectedProvinces.length > this.#GetMaxProvincesLengthForCommand()) this.SelectedProvinces.splice(this.#GetMaxProvincesLengthForCommand());

            if (!this.SelectedCountry) this.SelectedCountry = this.MapView.GamePhase.GetCountryWithUnitInProvince(this.SelectedProvinces[0]);
            // if (!this.SelectedCountry) this.SelectedCountry = this.MapView.GamePhase.GetSupplyCenterOwner(this.SelectedProvinces[0]);
            if (!this.SelectedCountry) this.SelectedCountry = this.PreviousCountry;

            color = this.SelectedCountry ? myHub.ColorScheme.CountryColors[this.SelectedCountry].ToRGBString() : "cyan";
            backcolor = this.SelectedCountry ? myHub.ColorScheme.CountryBackColors[this.SelectedCountry].ToRGBString() : "cyan";
            text = this.SelectedCountry ? this.SelectedCountry.substring(0, 1) + ": " : "";

            let bOrderIsFinished = false;
            let oar = new gmOrderAndResolution(this.SelectedProvinces[0], null, null);
            let bNeedsCoast = !this.SelectedToCoast && ["m", "sm", "c"].includes(this.SelectedCommand) && this.SelectedProvinces.length == this.#GetMaxProvincesLengthForCommand() && [ProvinceEnum.Stp, ProvinceEnum.Spa, ProvinceEnum.Bul].includes(this.SelectedProvinces[this.SelectedProvinces.length - 1]);

            switch (this.SelectedCommand) {
                case "h":
                    let hord = new gmOrderHold(null);
                    oar.Order = hord;
                    bOrderIsFinished = true;
                    break;

                case "m":
                    let mord = new gmOrderMove(null);
                    if (this.SelectedProvinces.length > 1) {
                        mord.ToLocation = new gmLocation(this.SelectedProvinces[1], this.SelectedToCoast);
                        bOrderIsFinished = !bNeedsCoast;
                    }
                    oar.Order = mord;
                    break;

                case "sh":
                    let shord = new gmOrderSupportHold(null);
                    if (this.SelectedProvinces.length > 1) {
                        shord.HoldLocation = new gmLocation(this.SelectedProvinces[1], null);
                        bOrderIsFinished = true;
                    }
                    oar.Order = shord;
                    break;
                ...
                case "sm":
                    let smord = new gmOrderSupportMove(null);
                    if (this.SelectedProvinces.length > 1) smord.FromLocation = new gmLocation(this.SelectedProvinces[1], null);
                    if (this.SelectedProvinces.length > 2) {
                        smord.ToLocation = new gmLocation(this.SelectedProvinces[2], this.SelectedToCoast);
                        bOrderIsFinished = !bNeedsCoast;
                    }
                    oar.Order = smord;
                    break;
                ...
                case "c":
                    let cord = new gmOrderConvoy(null);
                    if (this.SelectedProvinces.length > 1) cord.FromLocation = new gmLocation(this.SelectedProvinces[1], null);
                    if (this.SelectedProvinces.length > 2) {
                        cord.ToLocation = new gmLocation(this.SelectedProvinces[2], this.SelectedToCoast);
                        bOrderIsFinished = !bNeedsCoast;
                    }
                    oar.Order = cord;
                    break;

                case "X": this.#ClearOrder(); this.#UpdateDisplay(); return;

                default:
                    break;
            }

            if (this.SelectedCountry && bOrderIsFinished) {
                this.#FinishOrder(oar);
                return;
            }
            text += oar.ToString();
        }
        ...
        if (!this.#OrderSVG) {
            this.#OrderSVG = new dbnSVGWithBorder(this.MapView);
            this.#TextSVG = this.#OrderSVG.AddText("", 0, 0, "black", "font-size:20px");
            this.#OrderSVG.MoveTo(new dbnPoint(60, 70));

            // console.log("Doing xx");
            // let bw = 10;
            // this.MapView.AddRectangle(100, 100, 130, 56, "blue", bw + 2, "none");
            // var xx = new dbnSVGWithBorder(this.MapView);
            // xx.AddCircle(0, 0, 5, "black", "2", "none");
            // xx.AddText("WHANK???", -30, 30, "green", "font-weight: bold");
            // xx.AddLine(-30, 20, 100, 20, "red", 10);
            // xx.AddLine(75, -6, 75, 50, "red", 10);
            // xx.Border.strokeWidth = bw;
            // xx.Border.rx = 0;
            // xx.SetSizeAndCenterContents(130, 56);
            // // xx.FitToContents(0, 0, 0, 0);
            // xx.MoveTo(new dbnPoint(100, 100));
            // console.log(xx.GetRect(), xx.getBBox(), xx.Border.getBBox());
            // console.log("Finishing xx");
        }

        if (text) {
            this.#OrderSVG.visible = true;
            this.#TextSVG.textContent = text;
            this.#OrderSVG.Border.fill = backcolor;
        } else {
            this.#OrderSVG.visible = false;
        }

        this.#UpdateCommands();
        this.#UpdateCountryButtons();
        this.#UpdateToCoastButtons();

        if (this.#OrderSVG.visible) this.#OrderSVG.FitToContents(4, 4, 4, 4);
    }

    #UpdateCommands() {
        let commands = ["h", "m", "sm", "sh", "c", "X"];

        const FontColor = "white";
        const UnselectedBackColor = "black";
        const SelectedBackColor = "red";
        const ButtonSize = new dbnSize(18, 14);
        const Spacing = 4;

        if (!this.#CommandButtons) {
            this.#CommandButtons = {};
            commands.forEach((x, i) => {
                let but = new dbnSVGButton(this.MapView);
                but.visible = false;
                let buttext = but.AddText(x.toUpperCase(), 0, 0, FontColor, "font-weight: normal; font-size:10px");
                but.SetSizeAndCenterContents(ButtonSize);
                this.#CommandButtons[x] = but;
                but.onclick = (() => { this.SelectedCommand = x; this.#UpdateDisplay(); }).bind(this);
                if (x == "X") {
                    but.Border.stroke = "red";
                    // buttext.fill = "red";
                    buttext.style.fontWeight = "bold";
                }
            });
        }

        if (this.SelectedCommand && this.SelectedProvinces && this.SelectedProvinces.length > 0 && this.MapView.GamePhase) {
            let pd = this.MapView.MapData.ProvinceData[this.SelectedProvinces[0]];
            let uwl = this.MapView.GamePhase.GetUnitWithLocation(this.SelectedProvinces[0]) ?? pd.GetDefaultUnitWithLocation();
            var pt = dbnPoint.FromNumberArray(pd.GetLocationForUnit(uwl));

            commands.forEach((x, i) => {
                let btn = this.#CommandButtons[x];

                let offpt = new dbnPoint(1, 1).MultiplyBy(-this.MapView.MapStyle.UnitSize / 2);
                switch (x) {
                    case "h": break;
                    case "m": offpt = offpt.WithOffset(0, -ButtonSize.Height - Spacing); break;
                    case "sm": offpt = offpt.WithOffset(-ButtonSize.Width - Spacing, 0); break;
                    case "sh": offpt = offpt.WithOffset(ButtonSize.Width + Spacing, 0); break;
                    case "c": offpt = offpt.WithOffset(0, ButtonSize.Height + Spacing); break;
                    case "X": offpt = offpt.WithOffset(ButtonSize.Width + Spacing, -ButtonSize.Height - Spacing); break;
                    default: throw "unknown command";
                }
                btn.MoveTo(pt.WithOffset(offpt));
                btn.visible = true;
                btn.BackColor = x == this.SelectedCommand ? SelectedBackColor : UnselectedBackColor;
            });

        } else {
            commands.forEach((x, i) => this.#CommandButtons[x].visible = false);
        }
    }
    ...
    #UpdateCountryButtons() {
        ...
        if (!this.#CountryButtons) {
            this.#CountryButtons = {};
            Object.values(CountryEnum).forEach(x => {
                let but = new dbnSVGButton(this.#OrderSVG);
                but.visible = false;
                let buttext = but.AddText(x.substring(0, 1), 0, 0, FontColor, "font-weight: normal; font-size:10px");
                but.SetSizeAndCenterContents(12, 12);
                // but.FitToContents(2, 2, 2, 2);
                this.#CountryButtons[x] = but;
                but.onclick = (() => { this.SelectedCountry = x; this.#UpdateDisplay(); }).bind(this);
            });

            this.#ReplaceExistingButton = new dbnSVGButtonWithText(this.#OrderSVG);
            this.#ReplaceExistingButton.BackColor = "black";
            this.#ReplaceExistingButton.AutoSize = true;
            this.#ReplaceExistingButton.Margin = 1;
            this.#ReplaceExistingButton.TextElement.fill = "white";
            this.#ReplaceExistingButton.TextElement.style = "font-weight: normal; font-size:10px";
            this.#ReplaceExistingButton.textContent = "Replace existing";
            this.#ReplaceExistingButton.onclick = (() => { this.ReplaceExisting = !this.ReplaceExisting; this.#UpdateDisplay(); }).bind(this);
        }

        if (this.SelectedCommand && this.SelectedProvinces && this.SelectedProvinces.length > 0 && this.MapView.GamePhase) {
            var textrect = this.#TextSVG.GetRect();
            var pt = textrect.Location.WithOffset(0, textrect.Size.Height + 3);

            Object.values(CountryEnum).forEach((x, i) => {
                let btn = this.#CountryButtons[x];
                btn.visible = true;
                btn.MoveTo(pt.WithOffset(15 * i, 0));
                btn.BackColor = x == this.SelectedCountry ? SelectedBackColor : UnselectedBackColor;
            });

            this.#ReplaceExistingButton.style.textDecoration = this.ReplaceExisting ? "none" : "line-through";
            let existing = this.GamePhase.GetOrdersForProvince(this.SelectedProvinces[0]);
            this.#ReplaceExistingButton.visible = existing ? Object.values(existing).length > 0 : false;
            this.#ReplaceExistingButton.MoveTo(this.#ReplaceExistingButton.visible ? pt.WithOffset(0, 15) : new dbnPoint(0, 0));

        } else {
            Object.values(CountryEnum).forEach(x => this.#CountryButtons[x].visible = false);
        }
    }

    #UpdateToCoastButtons() {
        const FontColor = "white";
        const UnselectedBackColor = "black";
        const SelectedBackColor = "red";

        if (!this.#ToCoastButtons) {
            this.#ToCoastButtons = {};
            Object.values(ProvinceCoastEnum).forEach(x => {
                let but = new dbnSVGButton(this.MapView);
                but.visible = false;
                let buttext = but.AddText(x, 0, 0, FontColor, "font-weight: normal; font-size:10px");
                but.FitToContents(2, 2, 2, 2);
                this.#ToCoastButtons[x] = but;
                but.onclick = (() => { this.SelectedToCoast = x; this.#UpdateDisplay(); }).bind(this);
            });
        }

        if (this.SelectedCommand && this.SelectedProvinces && this.SelectedProvinces.length == this.#GetMaxProvincesLengthForCommand() && this.MapView.GamePhase) {
            let prov = this.SelectedProvinces[this.SelectedProvinces.length - 1];
            let coasts = [];
            switch (prov) {
                case ProvinceEnum.Stp: coasts.push(ProvinceCoastEnum.None, ProvinceCoastEnum.nc, ProvinceCoastEnum.sc); break;
                case ProvinceEnum.Spa: coasts.push(ProvinceCoastEnum.None, ProvinceCoastEnum.nc, ProvinceCoastEnum.sc); break;
                case ProvinceEnum.Bul: coasts.push(ProvinceCoastEnum.None, ProvinceCoastEnum.ec, ProvinceCoastEnum.sc); break;
                default: break;
            }

            coasts.forEach(x => {
                let btn = this.#ToCoastButtons[x];
                let pd = this.MapView.MapData.ProvinceData[prov];
                let uwl = new gmUnitWithLocation(x == ProvinceCoastEnum.None ? UnitTypeEnum.Army : UnitTypeEnum.Fleet, new gmLocation(prov, x));
                var pt = dbnPoint.FromNumberArray(pd.GetLocationForUnit(uwl));
                btn.visible = true;
                btn.MoveTo(pt);
                btn.BackColor = UnselectedBackColor;
            });

        } else {
            Object.values(ProvinceCoastEnum).forEach(x => this.#ToCoastButtons[x].visible = false);
        }
    }
}
...
class dbnMapView extends dbnSVG {
    ...
    set AllowEdits(value) { this.#AllowEdits = value; this.#Draw; }
    ...
    #AddNavigationButtons() {
        ...
        //S01
        var btn = new dbnSVGButtonWithText(this);
        ...
        btn.SetMarginAll(1);
        btn.style.fontWeight = "bold";
        btn.textContent = this.Game.GamePhases[this.Game.GamePhases.length - 1].PhaseTextShort;
        btn.MoveTo(pt);
    ...
    #DrawProvinces() {
        ...
        Object.values(this.MapData.ProvinceData).forEach(x => {
            ...
            if (x.ProvinceType == ProvinceTypeEnum.Water) {
                ...
                color = owner ? colors.CountryBackColors[owner] : colors.NeutralColor;
            }
            fill = color.ToRGBString();
            hoverfill = color.MixWith(dbnColor.Black, 0.2).ToRGBString();
            ...
            if (orddiv.domelement.childNodes.length > 0 && this.ViewingMode != GameViewingModeEnum.ProvincesOnly) {
                provincesvg.onmousemove = (e) => { dbnSVGElement.ShowTooltip(e, orddiv); if (this.AllowEdits) provincesvg.style.fill = hoverfill; }
                provincesvg.onmouseout = (e) => { dbnSVGElement.HideTooltip(); if (this.AllowEdits) provincesvg.style.fill = fill; }
                provincesvg.onclick = (e) => { this.OnProvinceSelect.Raise(x.Province); }
            }
    ...
    //#region Draw Player Names

    #DrawPlayerNames() {

        if (this.Game?.Players) {
            let mapsize = this.GetRect().Size;
            let tester = this.AddText("", 0, 0, "white", "font-size:16px;");
            let border = 2;
            let mapedgebuffer = border / 2;
            let padding = 2;
            ...
            Object.entries(this.Game.Players).forEach(x => {
                ...
                tester.textContent = playername;
                // let text = this.AddText(playername, 0, 0, "black", "font-size:20px; font-weight: bold");
                let rect = tester.GetRect();

                let pt = new dbnPoint(0, 0);
                let halign = "l";
                let valign = "t";

                switch (country) {
                    case CountryEnum.Austria: pt = new dbnPoint(mapsize.Width / 2, mapsize.Height - mapedgebuffer); halign = "c"; valign = "b"; break;
                    case CountryEnum.England: pt = new dbnPoint(mapedgebuffer, 95); break;
                    case CountryEnum.France: pt = new dbnPoint(mapedgebuffer, 120); break;
                    case CountryEnum.Germany: pt = new dbnPoint(mapsize.Width / 2, mapedgebuffer); halign = "c"; break;
                    case CountryEnum.Italy: pt = new dbnPoint(mapedgebuffer, mapsize.Height - mapedgebuffer); valign = "b"; break;
                    case CountryEnum.Russia: pt = new dbnPoint(mapsize.Width - mapedgebuffer, 90); halign = "r"; break;
                    case CountryEnum.Turkey: pt = new dbnPoint(mapsize.Width - mapedgebuffer, mapsize.Height - mapedgebuffer); halign = "r"; valign = "b"; break;
                    default: break;
                }

                switch (halign) {
                    case "l": pt = pt.WithOffset(0, 0); break;
                    case "c": pt = pt.WithOffset(-rect.Size.Width / 2 - padding, 0); break;
                    case "r": pt = pt.WithOffset(-rect.Size.Width - 2 * padding, 0); break;
                    default: throw "unknown halign";
                }
                switch (valign) {
                    case "t": pt = pt.WithOffset(0, 0); break;
                    case "m": pt = pt.WithOffset(0, -rect.Size.Height / 2 - padding / 2); break;
                    case "b": pt = pt.WithOffset(0, -rect.Size.Height - padding); break;
                    default: throw "unknown valign";
                }

                let box = this.AddRectangle(pt.X, pt.Y, rect.Size.Width + 2 * padding, rect.Size.Height + padding, "black", border, myHub.ColorScheme.CountryColors[country].ToRGBString());

                pt = pt.WithOffset(padding, padding + border / 2);
                let text = this.AddText(playername, pt.X, pt.Y, tester.fill, tester.style.cssText);

                box.SetPointerEventsNone();
                text.SetPointerEventsNone();
            });

            this.removeChild(tester);
    ...
    #GetMoveSegment(fromprovince, tolocation) {
        ...
        var moveline = new dbnLineSegment(pdFrom.GetLocationForUnit(uwlFrom), pdTo.GetLocationForUnit(uwlTo) ?? pdTo.GetLocationForUnit(pdTo.GetDefaultUnitWithLocation()));
...
class dbnMapStyle_DBN_2022_2 extends dbnMapStyle {
  ...
  */
    ...
    DrawSupportHold(country, ptFrom, ptTo, succeeded) { // circled held unit
        ...
        //Triangle
        // var base = this.UnitSize / 2;
        // var tip = 2;
        // var ptCenter = line.MiddlePoint;
        // let triangle = new dbnPointSet(0, base);
        // triangle.AddFromLast(0, -2 * base);
        // triangle.AddFromLast(line.Length, base - tip);
        // triangle.AddFromLast(0, 2 * tip);
        // triangle.Rotate(line.AngleToHorizontalInRadians);
        // // var trisvg = this.SVG.AddPolygon(ptFrom.X, ptFrom.Y, triangle.Points.map(x => x.ToArray()), "none", 0, "black");
        // triangle.Translate(ptFrom);
        // var d = triangle.Points[0].ToPath("M") + triangle.Points[1].ToPath("L") + ptCenter.ToPath("Q") + triangle.Points[2].ToPath("") + triangle.Points[3].ToPath("L") + ptCenter.ToPath("Q") + triangle.Points[0].ToPath("") + "Z";

        // var trisvg = this.SVG.AddPath(d, "black", 0, "black");
        // trisvg.SetPointerEventsNone();
        // trisvg = this.SVG.AddPath(d, "white", 0, "white");
        // trisvg.SetPointerEventsNone();

        //marker
        // var base = this.UnitSize / 2;
        // let triangle = new dbnPointSet(2 * base, base);
        // triangle.AddFromLast(0, -2 * base);
        // triangle.Rotate(line.AngleToHorizontalInRadians);
        // triangle.Translate(ptFrom);
        // var d = triangle.Points[0].ToPath("M") + triangle.Points[1].ToPath("L");
        // var trisvg = this.SVG.AddPath(d, "black", 4, "none");
        // trisvg.SetPointerEventsNone();
...
class dbnScoreboard extends dbnBaseTable {
    ...
    #UpdateDisplay() {
        ...
        Object.values(CountryEnum).forEach((country, i) => {
            ...
            if (Math.floor(score) != score) score = Math.round(10 * score) / 10;
