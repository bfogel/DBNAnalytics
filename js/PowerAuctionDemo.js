...
var myManager = PowerBidManager.GetManagerForCompetition(3051);//DBNI 2020

var cardTitle = dbnHere().addCard();
var cardDemo = dbnHere().addCard();

var divOutput = new dbnDiv();
var divBoards = Array.from(Array(myManager.BoardsPerRound), x => divOutput.addDiv());
...
function LoadPrevious(pRound) {
    ...
    myManager.BoardNamesByRound[1] = Array.from(Array(2), (x, i) => "R" + pRound + "B" + (i + 1));
...
function ResolveBids() {
    ...
    var boards = [];
    var boardnames = myManager.BoardNamesByRound[iRound];
    boardnames.forEach((name, boardi) => boards[boardi] = {});

    mPlayerBidInputs.forEach(pbi => pbi.ClearResults());

    var auction = myManager.MakeNewAuction(iRound);
    auction.Resolve();
    mPlayerBidInputs.forEach(pbi => {
        pbi.UpdateDisplay();
        var bs = pbi.BidSet;
    ...
    boardnames.forEach((name, boardi) => {
        ...
        myManager.PowerNames.forEach((powername, iRow) => {
